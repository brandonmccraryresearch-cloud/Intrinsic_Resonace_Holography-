...
class CacheEntry:
    ...
    def to_dict(self) -> Dict:
        ...
        return {"key": self.key, "data": self.data, "timestamp": self.timestamp, "ttl": self.ttl}

    @classmethod
    def from_dict(cls, d: Dict) -> CacheEntry:
        """Deserialize from dictionary."""
        return cls(key=d["key"], data=d["data"], timestamp=d["timestamp"], ttl=d.get("ttl"))
...
class CacheManager:
    ...
    def set(self, key: str, data: Any, ttl: Optional[float] = None):
        ...
        entry = CacheEntry(key=key, data=data, timestamp=time.time(), ttl=ttl)
    ...
    def info(self) -> Dict:
        ...
        # Estimate size
        total_size = sum(len(json.dumps(e.data)) for e in entries)
...
def create_cache_manager(
    cache_dir: str = "data/cache/experimental", ttl_hours: float = 24.0
) -> CacheManager:
    """
    ...
    """
    return CacheManager(cache_dir=cache_dir, default_ttl=ttl_hours * 3600)
